 ...
 */
avl_t *avl_full(avl_t *tree, avl_t *parent, int value)
{
	avl_t *new_node;
	...
	if (tree == NULL)
	{
		...
		return (avl_full(tree->left, tree, value));
	else
		return (avl_full(tree->right, tree, value));
}

/**
 * balancer - rebalance an AVL tree if needed
 *
...
void balancer(avl_t **root, avl_t *tree, int value)
{
	int new_balance;
...
avl_t *avl_insert(avl_t **tree, int value)
{
	...
	new_node = avl_full(*tree, NULL, value);
	...
	{
		balancer(tree, predecessor, value);
